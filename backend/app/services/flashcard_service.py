...
async def generate_flashcards(
    ...
    # Gọi AI (đã dùng asyncio.to_thread trong ai_service)
    cards_data = await ai_service.generate_flashcards(
        raw_text=raw_text,
        num_flashcards=body.num_flashcards,
    )

    # Lưu vào DB (to_thread để không block)
    card_records = [
        {
    ...
    result = await asyncio.to_thread(
        lambda: admin.table("flashcards").insert(card_records).execute()
    )
...
async def get_flashcards_by_document(
    ...
    """
    # Kiểm tra quyền (to_thread để không block)
    doc_result = await asyncio.to_thread(
        lambda: admin.table("documents")
            .select("id")
            .eq("id", document_id)
